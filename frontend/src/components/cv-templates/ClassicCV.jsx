...
// // Editable text - supports richText via global SelectionToolbar
// const EF = memo(({ value = "", onChange, placeholder, style, multiline, isEdit, textColor, richText = false }) => {
//     if (!isEdit) {
//         if (richText && value) return <div dangerouslySetInnerHTML={{ __html: value }} style={{ color: textColor, whiteSpace: "pre-wrap", ...style }} />;
//         return value ? <div style={{ color: textColor, whiteSpace: "pre-line", ...style }}>{value}</div> : null;
//     }
//     if (richText) return <RichTextField value={value} onChange={onChange} placeholder={placeholder} multiline={multiline} style={{ color: textColor, ...style }} isEdit={isEdit} />;
...
//                     </div>
//                     <EF value={exp.description} onChange={v => onChange("description", v)} placeholder="Mô tả công việc..." multiline richText={true} style={{ marginTop: 4, color: textColor }} isEdit={isEdit} />
//                 </>
//             )
//         },
//         education: {
...
//                     <EF value={aw.description} onChange={v => onChange("description", v)} placeholder="Mô tả..." richText={true} style={{ color: textColor, fontSize: 12 }} isEdit={isEdit} />
//                 </>
//             )
//         },
//         certifications: {
...
//                     <EF value={act.description} onChange={v => onChange("description", v)} placeholder="Mô tả hoạt động..." multiline richText={true} style={{ color: textColor }} isEdit={isEdit} />
//                 </>
//             )
//         },
//     }), [isEdit, accent]);
...
//         setLocalTitles(sectionTitles);
//         lastPropData.current = data;
//     }, [data, sectionOrder, sectionTitles, shouldUseSampleData, forceReset]);
...
import { useState, useEffect, useCallback, useRef, memo, useMemo } from 'react';
import ItemControls from "./../common/ItemControls/ItemControls";
import { registerActiveField, clearActiveField, saveGlobalSelection, restoreGlobalSelection } from "../common/UnifiedToolbar/UnifiedToolbar";
...
const InlineEdit = memo(({ value = "", onChange, placeholder, style, styleConfig = {}, onStyleChange, multiline = false }) => {
    ...
    const restoreSelection = useCallback(() => {
        if (savedSelRef.current && ref.current) {
            const sel = window.getSelection();
            sel.removeAllRanges();
    ...
    const handleBlur = useCallback((e) => {
        // Chỉ clear nếu không click vào toolbar
        setTimeout(() => {
    ...
    // 🔥 Xử lý danh sách số tự động tăng
...
// ─── Simple Input (cho preview mode) ───────────────────────────────────────────
const SimpleInput = memo(({ value = "", onChange, placeholder, style, type = "text" }) => {
    const [local, setLocal] = useState(value);
    const t = useRef(null);
    const onChangeRef = useRef(onChange);
    ...
    useEffect(() => { onChangeRef.current = onChange; }, [onChange]);
    useEffect(() => {
        if (value !== local && document.activeElement !== document.activeElement) setLocal(value);
    }, [value]);
...
const SectionHeader = memo(({ sectionKey, title, isEdit, index, total, accent, textColor, onTitleChange, onMove, onDelete, styleConfig = {}, onStyleChange }) => {
    const sectionTitleStyle = {
        fontSize: 13,
        fontWeight: "bold",
        color: textColor,
...
export default function ClassicCV({
    ...
    const AddBtn = useCallback(({ onClick, label = "+ Thêm" }) => (
        <button onClick={onClick} style={{ fontSize: 11, color: textColor, background: "#fff", border: `1px dashed ${accent}`, borderRadius: 4, padding: "4px 10px", cursor: "pointer", marginTop: 4, width: "100%" }}>
            {label}
        </button>
    ), [accent, textColor]);

    return (
        <div id="cv-paper" className="cv-paper" style={{ width: "100%", maxWidth: "794px", minHeight: "1123px", background: bgColor, fontFamily: styleConfig.fontFamily || "Lato, sans-serif", fontSize: styleConfig.baseFontSize || 13, lineHeight: styleConfig.lineHeight || 1.6 }}>
            {/* Header - dùng HeaderEdit cho tất cả để có toolbar */}
            <div style={{ background: accentColor, padding: "32px 40px 24px", color: "#fff" }}>
                <HeaderEdit
                    ...
                    styleConfig={styleConfig}
                    onStyleChange={onStyleChange}
                    style={{ fontSize: 28, fontWeight: "bold", color: styleConfig.textColor === "#ffffff" ? "white" : styleConfig.textColor, marginBottom: 6, borderBottom: isEdit ? "1.5px dashed rgba(255,255,255,0.4)" : "none" }}
                />
                {isEdit ? (
                    <HeaderEdit
                        value={pi.portfolio || ""}
                        ...
                        styleConfig={styleConfig}
                        onStyleChange={onStyleChange}
                        style={{ fontSize: 14, color: textColor, marginBottom: 12, borderBottom: "1px dashed rgba(255,255,255,0.3)", width: "60%" }}
                    />
                ) : pi.portfolio ? (
                    <div style={{ fontSize: 14, color: textColor, marginBottom: 12 }}>
                        ...
                        <div key={key} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 12, color: textColor }}>
                            ...
                            {isEdit ? (
                                <HeaderEdit
                                    ...
                                    styleConfig={styleConfig}
                                    onStyleChange={onStyleChange}
                                    style={{ fontSize: 12, color: textColor, borderBottom: "1px dashed rgba(255,255,255,0.3)", width: 150 }}
                                />
                            ) : pi[key] ? (
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                            {isEdit ? (
                                <TitleEdit
                                    value={getTitle("summary")}
                                    onChange={v => updateTitle("summary", v)}
                                    style={{ fontSize: 13, fontWeight: "bold", letterSpacing: 1.5, textTransform: "uppercase", borderBottom: `2px solid ${accent}`, paddingBottom: 4, flex: 1 }}
                                    textColor={textColor}
                                    styleConfig={styleConfig}
                                    onStyleChange={onStyleChange}
                                />
                            ) : (
                                <div style={{ fontSize: 13, fontWeight: "bold", letterSpacing: 1.5, textTransform: "uppercase", borderBottom: `2px solid ${accent}`, paddingBottom: 4, flex: 1 }}>
                                    {isHTML(getTitle("summary")) ? (
                                        <span dangerouslySetInnerHTML={{ __html: getTitle("summary") }} />
                                    ) : (
                                        getTitle("summary")
                                    )}
                                </div>
