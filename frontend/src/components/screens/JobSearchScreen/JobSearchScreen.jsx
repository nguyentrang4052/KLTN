...
function JobSearchScreen() {
    ...
    const salaryLabel = SALARY_OPTIONS.find(o => o.min === salaryMin && o.max === salaryMax)?.label;
    ...
    const allChips = [
        ...
        ...(salaryLabel && salaryLabel !== 'Tất cả' ? [{ cat: 'salary', value: 'salary', label: `💰 ${salaryLabel}` }] : []),
    ];
                            ...
                            {allChips.map(chip => (
                                <div key={`${chip.cat}-${chip.value}`} className="af-chip"
                                    onClick={() => {
                                        ...
                                        else if (chip.cat === 'salary') { setSalaryMin(0); setSalaryMax(0); setSalaryRange(100); goToPage(1, true); }
