...
let JobsService = class JobsService {
    ...
    async getJobs(dto, accountID) {
        const { keyword, locations, industryId, sort = 'newest', page = 1, limit = 9, jobType, experience, source, salaryMin, salaryMax, } = dto;
        ...
        if (jobType) {
            ...
            andConditions.push({
                OR: variants.map((v) => ({
                    jobType: { contains: v, mode: 'insensitive' },
                })),
            });
        }
        if (locations && locations.length > 0) {
            andConditions.push({
                OR: locations.map((loc) => ({
                    shortLocation: { contains: loc, mode: 'insensitive' },
                })),
            });
        }
        ...
        if (accountID) {
            ...
            if (user) {
                ...
                matchMap = Object.fromEntries(recs.map((r) => [r.jobID, r.matchPercent]));
            }
        }
    ...
    async getRecommendations(accountID, wasRecomputed = false) {
        const user = await this.prisma.user.findFirst({
            where: { accountID },
            select: { userID: true },
        });
    ...
    async logUserBehavior(accountID, jobID, action) {
        const user = await this.prisma.user.findFirst({
            where: { accountID },
        });
        if (!user)
    ...
    async incrementRecommendationQuota(accountID) {
        const user = await this.prisma.user.findFirst({
            where: { accountID },
            select: { userID: true },
        });
    ...
    async getSavedJobs(dto, accountID) {
        const user = await this.prisma.user.findUnique({ where: { accountID } });
    ...
    async getFilterOptions() {
        ...
        const [jobTypes, sources, locations, experiences, industries] = await Promise.all([
    ...
    async getTrendingKeywords() {
        ...
        const skillMap = Object.fromEntries(skillDetails.map((s) => [s.skillID, s.name]));
    ...
    async getFilterOptionsBySource(source) {
        const where = {
            isActive: true,
        ...
        for (const r of jobTypes) {
            ...
            for (const part of parts) {
                ...
                if (!VALID_JOB_TYPES.includes(normalized))
                    continue;
                jobTypeMap.set(normalized, (jobTypeMap.get(normalized) ?? 0) + r._count.jobID);
            }
        }
        const orderedJobTypes = VALID_JOB_TYPES.filter((v) => jobTypeMap.has(v)).map((v) => ({ value: v, count: jobTypeMap.get(v) }));
        return {
    ...
    async getJobMatch(accountID, jobID) {
        const user = await this.prisma.user.findFirst({
            where: { accountID },
            select: { userID: true },
        });
    ...
    async saveSearchHistory(accountID, keyword) {
    ...
    async getSearchHistory(accountID) {
    ...
    async getIndustryTrends() {
        ...
        const [industries, jobCounts, currentJobCounts, previousJobCounts, viewCounts, saveCounts, searchCounts,] = await Promise.all([
            this.prisma.industry.findMany({
            ...
            this.prisma.userBehavior.groupBy({
                ...
                for (const row of rows) {
                    ...
                    if (indID)
                        result.set(indID, (result.get(indID) ?? 0) + row._count.id);
                }
                return result;
            }),
    ...
    async getJobMatchPreview(accountID, jobID) {
        const user = await this.prisma.user.findFirst({
            where: { accountID },
            select: { userID: true },
        });
    ...
    async getJobMatchDetail(accountID, jobID) {
        const user = await this.prisma.user.findFirst({
            where: { accountID },
            select: { userID: true },
        });
        ...
        const userSkills = new Set(userSkillRows.map((s) => s.skill.name.toLowerCase()));
        ...
        const skillOverlap = jobSkills.filter((s) => userSkills.has(s.toLowerCase()));
        const skillGap = jobSkills.filter((s) => !userSkills.has(s.toLowerCase()));
        const industryMatch = !!(profile?.industry?.name &&
            job.industry?.name &&
            profile.industry.name.toLowerCase() === job.industry.name.toLowerCase());
        const expMatch = !!(profile?.experienceYear &&
            job.experienceYear &&
