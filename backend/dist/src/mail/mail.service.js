...
let MailService = class MailService {
    ...
    async sendAlertConfirmation(email, keyword) {
        await this.transporter.sendMail({
            from: `"GZCONNECT" <${this.config.get('MAIL_USER')}>`,
            to: email,
    ...
    async sendJobAlert(email, keyword, jobs) {
        const SOURCE_COLORS = {
            TopCV: '#00B14F',
            CareerLink: '#D0392A',
            CareerViet: '#1565C0',
...
<body style="margin:0; padding:0; background:#F5F0E8; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#F5F0E8; padding: 32px 16px;">
    ...
    async sendPaymentInvoice(email, data) {
...
<body style="margin:0;padding:0;background:#F5F0E8;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#F5F0E8;padding:32px 16px;">
    ...
    async sendSubscriptionExpiringSoon(email, data) {
        const fmt = (d) => new Date(d).toLocaleDateString('vi-VN', {
            day: '2-digit',
            month: '2-digit',
            year: 'numeric',
...
<body style="margin:0;padding:0;background:#F5F0E8;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#F5F0E8;padding:32px 16px;">
    ...
    async sendSavedJobDeadlineAlert(email, data) {
        const fmt = (d) => new Date(d).toLocaleDateString('vi-VN', {
            day: '2-digit',
            month: '2-digit',
            year: 'numeric',
